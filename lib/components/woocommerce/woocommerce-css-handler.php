<?php
/**
 * Adds the WooCommerce styles and the Customizer additions for WooCommerce.
 *
...
use RuyFialho\Developers\Customizer as customizer;
...
function enqueue_woocommerce_styles( $enqueue_styles ) {
	...
	$enqueue_styles['developers-woocommerce-styles'] = array(
		'src'     => CHILD_THEME_URL . '/lib/components/woocommerce/developers-woocommerce.css',
		'deps'    => '',
...
add_action( 'wp_enqueue_scripts', __NAMESPACE__ . '\adds_woocommerce_css' );
/**
...
function adds_woocommerce_css() {
	...
	if ( $woo_css ) {
		wp_add_inline_style( 'developers-woocommerce-styles', $woo_css );
	}
